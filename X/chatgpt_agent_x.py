...
def generate_tweet():
    ...
    response = ai_client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Generate a concise post with tips, news, or shortcuts for {post_topic}. Ensure it is under 200 characters including hashtags.",
            },
        ],
    )
    logger.info(
...
def generate_image_for_topic(post_topic: str):
    ...
    try:
        response = ai_client.images.generate(
            prompt=f"Create random isometric pixel art scene of a busy data center. Include elements such as {post_topic}, stacks of servers, cables, and networking equipment.",
            n=1,
            size="512x512",
        )
